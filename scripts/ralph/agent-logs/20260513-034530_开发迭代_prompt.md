...
- **Description**: 作为前端开发者 · 6 个 invites procedure · 含 invalidate 走 Approval Gates(super_admin 直批 stub)+ batchImport CSV chunk 100 + campaignFunnel 4 阶段漏斗。
- **Priority**: 20
- **Retry Count**: 0
- **Acceptance Criteria**:
...
- **教训** (来自 PRD-10+ADR-016 / ADMIN §7.6): 高风险必走 Approval Gates stub
  - 反例: `prisma.update() 直接`
  - 正确: `approval_requests.create({status:auto_executed/pending}) + audit`
- **教训** (来自 PRD-8 / batchImport chunk): CSV 批量插入必 chunk 100 · 不一次 insert 10k 行 OOM
  - 反例: `prisma.createMany({data: allRows})  // 10k 行 OOM`
  - 正确: `for (let i=0; i<rows.length; i+=100) await prisma.createMany({data: rows.slice(i, i+100)})`
- **教训** (来自 PRD-3 / CSV 解析): CSV 解析用 papaparse · 不手撸 split(",")
  - 反例: `csv.split('\n').map(l => l.split(','))  // 引号字段错`
  - 正确: `papaparse.parse(csv, {header: true}).data`
...
- **US-019**: invite_campaigns schema + campaign service — [OK] 已完成