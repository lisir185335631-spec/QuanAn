...
- **Story ID**: US-001
- **Title**: 合并 TD-012 + React Router v6 + 34 路由 config 懒加载
- **Description**: 删 account/step alias router · hooks 改调 ipAccounts/stepData · createBrowserRouter 34 路由 · React.lazy 分模块 chunk
- **Priority**: 1
- **Retry Count**: 0
- **Acceptance Criteria**:
  1. 删 apps/api/src/trpc/routers/{account,step}.ts · _app.ts 移除 import + spread
  2. useActiveAccount 改调 trpc.ipAccounts.switchActive · useStepData 改调 trpc.stepData.save · useEvolution 不变(独立 router 已就位)
  3. apps/web/src/router.tsx · createBrowserRouter 配 34 路由(9 step + 14 工具 + 6 新模块 + 3 辅助 + /ip-plan + /accounts)
  4. 全部路由用 React.lazy + Suspense fallback skeleton
  5. 嵌套路由 · Header layout 共享(outlet)· 不存在路径 → /404
  6. 首屏 < 2s · 路由切换 < 200ms · 同模块路由共享 chunk(如 /step/* 一个 chunk)
  7. Typecheck passes
  8. Tests pass · TD-012 合并不破坏现有 174 tests

### [WARN] 上次验证失败的反馈（优先阅读，针对性修复）：
medium · 路由 config 是 PRD-3 基础 · TD-012 顺带清
