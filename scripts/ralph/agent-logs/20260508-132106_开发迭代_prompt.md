...
- **Story ID**: US-005
- **Title**: /ip-plan 进度可视化 · IPProgressService 0/9 + 当前 step 高亮
- **Description**: IPProgressService 计算 9 step 完成数 · UI 9 step 进度条 + 高亮 + 反馈按钮 placeholder(P0 仅 trace)
- **Priority**: 5
- **Retry Count**: 1
- **Acceptance Criteria**:
  1. /ip-plan 页 · 9 step 进度条(完成 高亮金 / 当前 高亮蓝 / 待办 灰)· 0/9 默认
  2. 调 trpc.stepData.progress 拿 completedSteps 数组 · 渲染对应 step 高亮
  3. 切账号后 progress 重新 fetch(useQuery 自动 invalidate)
  4. 反馈按钮 placeholder · 全 step 页 + /ip-plan · 点击 → trpc.cost_log 写 traceId(P0 不真 evolve · 留 PRD-7)
  5. stepData 加载失败 → skeleton 占位 · 不 crash
  6. /ip-plan 首屏 < 1s
  7. Typecheck passes
  8. Tests pass · ≥ 4 unit(IPProgressService + 进度条组件)

### [WARN] 上次验证失败的反馈（优先阅读，针对性修复）：
[验证失败 - 第1次] 2026-05-08 13:20
- AC-4 失败：FeedbackButton 仅存在于 /ip-plan 页(IpPlan.tsx)，所有 11 个 step 页(Step1.tsx～Step9.tsx 含 Step2/3b/4b)均未导入或使用 FeedbackButton。AC 要求 "全 step 页 + /ip-plan" 都有反馈按钮。
- 其他 AC 全部通过：AC-1/2/3/5/6/7/8 ✓ · 192 tests pass · typecheck pass
- 建议修复方向：在所有 /pages/step/Step*.tsx 文件末尾添加 <FeedbackButton stepKey="stepN" />
