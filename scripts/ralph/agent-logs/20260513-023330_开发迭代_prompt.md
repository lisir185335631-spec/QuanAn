...
- **Description**: 作为法务 · 点击 [PDF 取证导出] 生成法务级 PDF · 含完整 trace 链路 + payloadHash + admin 签名页(待手签 + 司法盖章占位)。
- **Priority**: 18
- **Retry Count**: 0
- **Acceptance Criteria**:
...
- **教训** (来自 PRD-9+LD-A-3 / PRD-9 US-014): audit payloadHash 必 redact
  - 反例: `SHA-256(payload)`
  - 正确: `SHA-256(redact(payload,['password','token']))`
- **教训** (来自 PRD-7 / PDF 签名): 签名页 admin info 从 ctx.session 取 · 不 hardcode
  - 反例: `<Text>张三 admin</Text>`
  - 正确: `<Text>{requesterEmail} ({requesterRole})</Text>`
- **教训** (来自 LD-A-3 / PDF 防篡改): PDF footer SHA-256 hash · 法务可重算 · 缺则不可信
...
- **US-016**: adminRouter.audit 5 procedure(byTraceId/byUserId/byAdminId/search/exportPdf) — [OK] 已完成