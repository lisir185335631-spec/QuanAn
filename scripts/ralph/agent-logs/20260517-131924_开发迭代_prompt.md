...
- **Description**: 作为用户,我想进入 /step/1 看到 1:1 复刻 aiipznt 的行业选择 page:顶部 step 标签 + H1 + 副标 + 搜索框 + 6 tab + 56 emoji 双行卡 grid · 能按 tab 筛选 + 搜索框实时过滤(by name + keywords)
- **Priority**: 5
- **Retry Count**: 0
- **Acceptance Criteria**:
...
[TD-HINT] 执行前 grep .agents/tech-debt.json status=open · 关注 Step1.tsx scope
...
- **US-001**: ★ foundation · industries.ts 56 行业 5 大类常量(字面锁 · 数字锁) — [OK] 已完成
