...
- **Story ID**: US-005
- **Title**: shadcn 12 基础组件 + Header 三 dropdown
- **Description**: As IP 起号者 · I want Header 显示 用户头像 / IP 账号 / 工具入口 三 dropdown · So that 推进 ARCHITECTURE §3.6.4 主应用骨架 P0 退出条件
- **Priority**: 5
- **Retry Count**: 1
- **Acceptance Criteria**:
  1. shadcn 12 基础组件就位(button / input / dropdown-menu / avatar / dialog / sheet / separator / scroll-area / toast / tooltip / tabs / select)· copy-paste 模式不依赖 npm shadcn 包
  2. Header 用户头像 dropdown · 显示 mock 用户(name=Mock User · email=mock@quanqn.dev · avatar 占位)· 含 logout 入口
  3. Header IP 账号 dropdown · 显示 mock 列表(2-3 个示例账号)+ '+ 新建账号' 入口
  4. Header 工具入口 dropdown · 显示 14 工具页 + 6 新模块占位(参 ARCHITECTURE §3.6.4 表)
  5. 0 React warning · 0 hydration mismatch(React 18 strict 模式 · 用 agent-browser 验证)
  6. 窄屏 ≤640px 下 · 三 dropdown 隐藏 · 显示 hamburger menu · 点开抽屉(sheet 组件)
  7. Header 首次渲染 < 100ms · dropdown 展开动画 < 200ms
  8. Typecheck passes
  9. Tests pass · E2E playwright Header 三 dropdown 可见 + 点开

### [WARN] 上次验证失败的反馈（优先阅读，针对性修复）：
medium · shadcn 用 copy-paste 不 npm install · radix-ui 是底层 dep
