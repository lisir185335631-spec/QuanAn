...
- **Story ID**: US-005
- **Title**: users 表 lastLoginAt + lastLoginIp 字段 + 4 admin indexes
- **Description**: 作为开发者 · users 表加 lastLoginAt + lastLoginIp(VarChar 45)+ 4 indexes(role/plan/industry/lastLoginAt)· auth middleware 每次成功登录更新 · 这样 US-007 用户管理列表能高效筛选 + 排序。
- **Priority**: 5
- **Retry Count**: 1
- **Acceptance Criteria**:
  1. prisma/schema.prisma model User 加 2 字段 lastLoginAt DateTime? + lastLoginIp String? @db.VarChar(45)
  2. model User 加 @@index([role]) + @@index([plan]) + @@index([industry]) + @@index([lastLoginAt(sort: Desc)])
  3. pnpm prisma migrate dev --name add_user_lastlogin_indexes · migration 含 ALTER TABLE users + 4 CREATE INDEX
  4. psql 实跑 \d users 看到新字段 + 4 indexes · 既有用户 last_login_at IS NULL 允许
  5. users 表仍 RLS ON (主应用 18 表 LD-A-2)· relrowsecurity=t
  6. apps/api/src/middleware/auth.ts 登录成功后 prisma.user.update({lastLoginAt: new Date(), lastLoginIp: ctx.req.ip})
  7. auth update 失败(用户删/DB 断)· try/catch · log warning · 不阻塞登录流程
  8. lastLoginIp 输入 IPv6 长度 ≤ 45 · 不抛 SQL truncation
  9. packages/schemas/user.schema.ts zod schema 加 lastLoginAt + lastLoginIp(optional)
  10. 单元测试 apps/api/src/middleware/__tests__/auth-lastlogin.test.ts · login 成功 update / 失败不 update / IPv4 写入 / IPv6 写入 4 test
  11. pnpm test + pnpm typecheck 0 error
  12. pnpm audit:redlines-admin 跑过 0 命中

### [WARN] 上次验证失败的反馈（优先阅读，针对性修复）：
[验证失败 - 第1次] 2026-05-12 18:51

## CONFIRMED(实测复现 + 命令 + 输出 · 100% 事实)

- 失败项1 (AC5): users 表 relrowsecurity=f，AC 要求 relrowsecurity=t
  · 复现命令: psql postgresql://return@localhost:5432/quanqn -c "SELECT relname, relrowsecurity FROM pg_class WHERE relname = 'users';"
  · 实际输出: users | f
  · 失败定位: manual_rls.sql 中 ENABLE ROW LEVEL SECURITY 列表未包含 users 表
  · 注：经确认，users 表从未在 manual_rls.sql 中开启 RLS，非 US-005 引入

- 失败项2 (X-6 零回归门禁): 全量测试 3 个文件失败
  · 复现命令: pnpm vitest run
  · 实际输出: Test Files 3 failed (rls-isolation.test.ts / evolution-threshold.test.ts / ip-progress-integration.test.ts)
  · 经 git stash 确认: stash 前后同样 3 failed，非 US-005 引入，为 pre-existing

## SUSPECTED(推测原因 · 仅候选 · 待复现验证)
- 无 SUSPECTED 项，两个失败均已 CONFIRMED 且为 pre-existing

## 建议修复方向(仅基于 CONFIRMED 给)
- AC5 修复: 在 manual_rls.sql 中加 ALTER TABLE users ENABLE ROW LEVEL SECURITY; 并执行
  注：若 Opus 认定是 pre-existing TD 可豁免，按 TD 豁免规则处理即可
- X-6 修复: 三个 integration test 失败为 pre-existing，建议 Opus TD 豁免处理

### [SHIELD] 历史反例 (anti_patterns) — 类似 story 的过往教训, 实现时主动避开:
- **教训** (来自 PRD-1 / PRD-1 US-002): IP 字段必限长 @db.VarChar(45) · IPv6 最长 45 chars · 默认 TEXT 浪费索引
  - 反例: `lastLoginIp String? (无 db type)`
  - 正确: `lastLoginIp String? @db.VarChar(45)`
- **教训** (来自 PRD-1 / auth 安全): auth update 失败不阻塞登录 · log warning + continue
  - 反例: `await prisma.user.update(...) 不 try/catch`
  - 正确: `try { await update } catch(e) { logger.warn(e) }`
