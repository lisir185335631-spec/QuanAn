...
- **Description**: 作为法务 / super_admin · 打开 admin.quanqn.com/audit · 输入 trace_id 反查 + 用户/admin 时间线切换 + 详情抽屉(prompt + response + payloadHash)+ PDF 取证导出按钮。
- **Priority**: 17
- **Retry Count**: 0
- **Acceptance Criteria**:
...
- **教训** (来自 LD-A-3 / payloadHash 显示): 时间线条目必显示 payloadHash · 法务取证用 · 不只 prompt/response
  - 反例: `<TimelineItem>{prompt}{response}</TimelineItem>`
  - 正确: `<TimelineItem>{prompt}{response}<Hash>{payloadHash}</Hash></TimelineItem>`
