...
- **Story ID**: US-006
- **Title**: 首页 READY TO START? CTA 区(Orbitron 大标题 + 鼓励语 + 主 CTA 立即启动)
- **当前 retryCount**: 0
- **Acceptance Criteria**:
  1. AC-1 · apps/web/src/pages/Home.tsx 加 ReadyToStartSection 函数 · section className="text-center py-16 mb-8" · H2 className="font-display text-4xl md:text-5xl font-black text-primary tracking-widest mb-4" 文字 READY TO START? · p className="font-cn text-lg text-muted-foreground mb-8" 文字 是时候开始了，IP 打造在等你 · <Link to="/step/1"><Button className="font-cn bg-primary text-primary-foreground hover:bg-primary/90 px-10 py-4 text-lg">立即启动 →</Button></Link>
  2. AC-2 · agent-browser 访问 / · 滚到底 · 验证 H2 + 鼓励语 + 主 CTA · click 立即启动 跳 /step/1
  3. AC-3 · 截图 screenshots/prd16-us006-home-cta.png · 同时 fullPage 截图 screenshots/prd16-us006-home-fullpage.png 验证整体首页 5 区(Hero + IpProgress + FunctionMatrix + Workflow + ReadyToStart)排列正确
  4. AC-4 · cd apps/web && pnpm typecheck pass · cd apps/web && pnpm test pass
