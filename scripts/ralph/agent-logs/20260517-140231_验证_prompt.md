...
- **Story ID**: US-004
- **Title**: Step 1 自定义输入行业 modal(shadcn Dialog · click 触发)
- **当前 retryCount**: 0
- **Acceptance Criteria**:
  1. Step1.tsx 加 <Dialog open={customModalOpen} onOpenChange={setCustomModalOpen}> · <DialogTrigger asChild><Button variant='link'>{STEP1_CUSTOM_TRIGGER_LABEL}</Button></DialogTrigger> · 关键 · 必须 asChild click 触发 · 严禁默认 hover
  2. DialogContent 用 glass-card · DialogHeader 含 DialogTitle font-display · Input maxLength=20 用 STEP1_CUSTOM_MODAL_PLACEHOLDER 字面
  3. DialogFooter 含 [取消] 按钮(variant='outline')+ [确认使用] 按钮(disabled if !customInput.trim() · bg-gradient-to-r from-primary to-primary/80)· 严禁 hardcode '确认' '取消' · 必须用 STEP1_CUSTOM_MODAL_CONFIRM/CANCEL 常量
  4. handleCustomConfirm · setCustomIndustry(customInput.trim()) + setSelectedIndustry(null)(custom 与 selected 互斥) + setCustomModalOpen(false) · 顶部状态卡显示 '已选择:{customIndustry}(自定义)' · emoji '✨' 占位
  5. 字面 grep · grep -F '自定义你的行业'/'请输入你的行业名称'/'确认使用'/'取消' apps/web/src/pages/Step1.tsx 每条命中(via 常量 import 间接)
  6. 使用 agent-browser · 打开 /step/1 · 点 '自定义输入行业' link · Dialog modal 弹出(不是 hover · 是 click)· 输入 '宠物服务' · 点 '确认使用'
  7. 使用 agent-browser · Modal 关闭 · 顶部状态卡显示 '已选择:宠物服务(自定义)' · 底部 CTA 变可点击
  8. Typecheck 通过
