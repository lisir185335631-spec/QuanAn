...
- **Description**: 作为运营/客服 · 打开 admin.quanqn.com/accounts 看跨账号列表 + 行业/平台/阶段分布 + 异常账号专属 Tab + 详情抽屉(9 步进度+进化档案+历史时间线+备注+异常 flag 列表)+ 强制冻结按钮。
- **Priority**: 11
- **Retry Count**: 0
- **Acceptance Criteria**:
...
- **教训** (来自 PRD-10+D-075 / PRD-10 tokens): DenseTable 用 @tanstack/react-virtual
  - 反例: `data.map(row=><tr>) 全 DOM`
  - 正确: `useVirtualizer({count,estimateSize:32})`
- **教训** (来自 PRD-6 / PRD-6 US-025): Recharts 数据点必含 fill var(--accent-X)
...
- **US-010**: adminRouter.accounts 6 procedure(list/detail/flag/unflag/addNote/forceFreeze) — [OK] 已完成