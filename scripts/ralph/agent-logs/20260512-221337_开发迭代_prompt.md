...
- **Description**: 作为运营 · [CSV 导出] 按钮把当前筛选后用户列表(可能 100k+ 行)stream 导出 · 后端 chunk 1000 防 OOM。
- **Priority**: 8
- **Retry Count**: 0
- **Acceptance Criteria**:
...
- **教训** (来自 PRD-8 / PRD-8 US-009): 大数据 export 用 stream + chunk · 防 OOM
  - 反例: `findMany({}) 100k 行一次 load`
  - 正确: `for offset; chunk 1000; pipe to res`
- **教训** (来自 PRD-3 / CSV escape): CSV 字段含逗号/引号必 escape · papaparse 或手撸
  - 反例: `rows.map(r => `${r.id},${r.email}`).join("\n")  // 含 "," 列错位`
  - 正确: `papaparse.unparse(rows) 或 row.replace(/"/g,'""').wrapInQuotes`
