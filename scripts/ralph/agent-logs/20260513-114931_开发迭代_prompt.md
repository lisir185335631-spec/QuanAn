...
- **Story ID**: US-007
- **Title**: deep_learn_review_queue + user_violation_log 2 schema
- **Description**: 作为开发者 · 域 ⑧ 2 张 admin 表(deep_learn_review_queue + user_violation_log)· 同 US-001 三联同步模式。
- **Priority**: 7
- **Retry Count**: 1
- **Acceptance Criteria**:
  1. prisma/schema.prisma 加 2 model 严格按 DATA-MODEL §13.6.E 行 3322-3375 · DeepLearnReviewQueue(userId/accountId/fileName/fileMime/fileSize/fileUrl/uploadedAt/autoScanResult/autoVerdict/status/reviewerAdminId?/reviewedAt?/rejectReason?/archiveId?)+ 4 indexes
  2. UserViolationLog(userId/violationType/count/lastViolationAt/lastReviewItemId?/warningCount/suspendedAt?/suspendedByAdminId?/suspendedReason?)+ @@unique([userId,violationType]) + 2 indexes
  3. pnpm prisma migrate dev --name add_deep_learn_review_tables · migration 2 CREATE TABLE
  4. manual_admin_rls.sql 加 2 行 ALTER TABLE DISABLE RLS · psql 实测 relrowsecurity=f
  5. scripts/audit-admin-rls-tables.sh 列表加 2 表 · 含 '域⑧ DeepLearning 审核' 注释
  6. audit-admin-rls-tables 18/18 admin tables 全过(13 PRD-10/11 + 3 PRD-12 US-001 + 2 新)0 mismatches
  7. zod schema deep-learn-review.schema.ts + user-violation.schema.ts · violationType enum('pii_upload'|'banned_content'|'trending_abuse'|'other')· status enum 同 trending
  8. @@unique([userId, violationType]) 重复触发 · service 层 upsert + count + 1 防 race
  9. 单元测试 zod schema · 8+ test
  10. pnpm test 全过 · pnpm typecheck 0 error
  11. Typecheck passes

### [WARN] 上次验证失败的反馈（优先阅读，针对性修复）：
[验证失败 - 第1次] 2026-05-13 03:50

## CONFIRMED(实测复现 + 命令 + 输出)

- 失败项(AC-5): audit-admin-rls-tables.sh 列表缺少 '域⑧ DeepLearning 审核' 注释
  · 复现命令: grep -n "域⑧\|DeepLearning" scripts/audit-admin-rls-tables.sh
  · 实际输出: (无匹配 — 命令返回空)
  · 失败定位: scripts/audit-admin-rls-tables.sh lines 28-30 — deep_learn_review_queue + user_violation_log 紧跟在 auto_review_rules 下方，无 '# 域⑧ DeepLearning 审核' domain 分隔注释

## SUSPECTED
(无)

## 建议修复方向
- 在 scripts/audit-admin-rls-tables.sh lines 28-29 之间插入注释行: # 域⑧ DeepLearning 审核
  正确位置: auto_review_rules 下方，deep_learn_review_queue 上方

### [SHIELD] 历史反例 (anti_patterns) — 类似 story 的过往教训, 实现时主动避开:
- **教训** (来自 PRD-11 retro M-3 / audit drift): audit-admin-rls-tables 同步加 2 表
  - 反例: `只 ALTER 漏 audit list`
  - 正确: `manual_rls + audit list + audit-self-test 三联`
- **教训** (来自 PRD-12 / upsert race): user_violation_log count + 1 用 upsert · 不 findFirst + create
  - 反例: `const e = await findFirst(); if (e) update; else create; (race condition)`
  - 正确: `upsert({where:{userId_violationType}, update:{count:{increment:1}}, create:{count:1}})`
