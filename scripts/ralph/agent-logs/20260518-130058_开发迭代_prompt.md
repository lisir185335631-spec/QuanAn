...
- **Story ID**: US-004
- **Title**: BrandingAgent 真 LLM(packaging + persona mode · for step3 + step3b)
- **Description**: 作为 QuanQn 用户 · 在 Step3 IP 定位 + Step3b 人设 · 真接 BrandingAgent · BrandingAgent.invokeLLM 真接 LLM · packaging mode(step3)+ persona mode(step3b)· system prompt 双 mode 严格区分 + responseFormat schema 各自验证 · cost_log 真存
- **Priority**: 4
- **Retry Count**: 0
- **Acceptance Criteria**:
  1. 修改 apps/api/src/specialists/BrandingAgent.ts · invokeLLM real call · packaging vs persona mode 严格区分 · 各自 system prompt + responseFormat schema · 不 hardcode model
  2. 新建 apps/api/src/specialists/__tests__/BrandingAgent.real-llm.test.ts · describe.skipIf · 2 单测 packaging + persona mode · 验证 result schema 严格对齐 fallback mock + isFallback=false + tokensUsed > 0
  3. Schema drift 防御 · 跟 US-003 同模式 · outputSchema.safeParse 严格 + double schema strategy
  4. Verify cost_log 真接 · BrandingAgent agentId 真存 · packaging/persona agentMode 字段也真存
  5. test_command · cd apps/api && pnpm vitest run src/specialists/__tests__/BrandingAgent.real-llm.test.ts(若 key 在 env)
  6. Typecheck passes · zero-regression

### [WARN] 上次验证失败的反馈（优先阅读，针对性修复）：
[TD-HINT] 同 US-003 · 防 hardcode model + Schema drift

### [SHIELD] 历史反例 (anti_patterns) — 类似 story 的过往教训, 实现时主动避开:
- **教训** (来自 prd-4-p3-ip-flow-9-steps / US-007 (TopicAgent hardcode model)): Specialist invokeLLM 严禁 hardcode model name · 必从 ctx / config 读 actual model · SHIELD REJ-003
  - 反例: `modelUsed: 'claude-sonnet-4-6'  // 反例`
  - 正确: `modelUsed: ctx.actualModel ?? this.config.execution.model_tier  // 正确`
- **教训** (来自 prd-20-real-llm-integration / US-003 (Schema drift)): packaging vs persona 双 mode 必各自独立 system prompt + outputSchema · 不允许混用
  - 反例: `system prompt = 'You are branding expert' (单 prompt 双 mode 用)  // 反例 · mode 行为不区分`
  - 正确: `systemPrompt = mode === 'packaging' ? '...账号包装...' : '...人设方案...'  // 正确`
- **教训** (来自 prd-20-real-llm-integration / US-003 (test skipIf)): real-llm.test.ts 必 describe.skipIf 无 key 时 SKIP · CI 默认 skip
  - 反例: `describe('BrandingAgent real LLM', () => ...)  // 反例 · CI 强跑`
  - 正确: `describe.skipIf(skipIfNoKey)('BrandingAgent real LLM', () => ...)`
