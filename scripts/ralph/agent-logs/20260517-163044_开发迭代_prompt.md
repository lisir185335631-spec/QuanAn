...
- **Description**: 作为质量保障,我需要跨 Step 1 → Step 3 → Step 3b 完整流程的 E2E 自动化测试 + 3 张截图,验证 stepData 跨 step 传递正确 + 字面 1:1 渲染 + 无控制台 error · 同时产出可重复 verify-prd-17.sh 跑全 PASS · zero-regression 硬门禁
- **Priority**: 11
- **Retry Count**: 0
- **Acceptance Criteria**:
...
[TD-HINT] 执行前 grep .agents/tech-debt.json status=open · 关注 e2e 类 TD
...
- **US-003**: Step 1 已选状态卡 + 主 CTA 跳转 — [OK] 已完成
- **US-004**: Step 1 自定义输入行业 modal(shadcn Dialog · click 触发) — [OK] 已完成
- **US-006b**: Step 3 page · 6 H3 输出区 + 每 H3 三按钮 + 顶部右侧 2 按钮 — [OK] 已完成
- **US-008**: Step 3b page · 3 textarea + 5 platform radio + 1 input + 5 H3 输出区 — [OK] 已完成