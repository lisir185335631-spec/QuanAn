...
[risk-upgrade] foundation due to downstream=2 (US-006 + US-007) + DATABASE_URL 全栈关键路径

### [SHIELD] 历史反例 (anti_patterns) — 类似 story 的过往教训, 实现时主动避开:
- **教训** (来自 QuanQn-base / REJ-008): 所有 query/mutate 必须按 account_id 过滤(per-tenant 隔离)· 即便 LD-009 全局表例外也要明确
  - 反例: `❌ prisma.entity.findMany() 不带 where account_id`
  - 正确: `✅ prisma.entity.findMany({ where: { accountId: ctx.activeAccountId } })`
- **教训** (来自 QuanQn-base / REJ-009): RLS 策略 12 业务表强制开 · admin 13 表 DISABLE · 不允许 prisma.$executeRaw 绕过 RLS
...
- **US-001**: 修复 monorepo 代码迁移产物(import 路径 + typecheck 干净) — [OK] 已完成