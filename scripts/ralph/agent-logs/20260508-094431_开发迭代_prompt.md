...
- **Retry Count**: 0
- **Acceptance Criteria**:
  1. apps/api/src/trpc/routers/knowledge.ts · 7 procedure(getRecommendations/getScriptCases/getFavorites/addFavorite/removeFavorite/getNotes/addNote)· mock
  2. apps/api/src/trpc/routers/trending.ts · 3 procedure(fetch/listByIndustry/listByStyle)· mock TrendingItem 列表
  3. apps/api/src/trpc/routers/invite.ts · 1 procedure(redeem)· 检查 InviteCode + 标 used + 关联 user
  4. invite.redeem 重复 redeem 同 code → 409 Conflict
  5. trending 走全局表(LD-009 例外)· 不需 RLS
  6. knowledge.addFavorite 走 per-account RLS · 跨账号收藏隔离
  7. Typecheck passes
  8. Tests pass · ≥ 11 unit

### [WARN] 上次验证失败的反馈（优先阅读，针对性修复）：
medium · knowledge mock · 实际 RAG 留 PRD-9 · trending mock 留 PRD-6 真抓取
