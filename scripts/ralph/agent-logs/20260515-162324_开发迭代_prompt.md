...
- **Story ID**: US-013
- **Title**: 套餐/行业灰度 + 用户白名单 + 任意 future feature_flag
- **Description**: 作为 super_admin · 我需要扩展 feature_flags 支持 3 复杂 flagType · percentage(灰度) / targeted(套餐/行业/用户名单) / 任意 future flag。每个 flag 修改走 dual approval。
- **Priority**: 13
- **Retry Count**: 0
- **Acceptance Criteria**:
  1. getFeatureFlagValue(flagKey, userId) flagType='percentage' · md5(${userId}:${flagKey}).slice(0,8) % 100 < rolloutConfig.percentage → enabled · deterministic · 同 user × flag 多次调一致
  2. getFeatureFlagValue(flagKey, userId, plan) flagType='targeted' · userId in rolloutConfig.target_users OR plan in rolloutConfig.target_plans → enabled · 否则 disabled
  3. rolloutConfig zod schema discriminatedUnion · z.object({flagType:z.literal('boolean')}) | percentage:number 0-100 | targeted: target_users z.number().int().array().optional() + target_plans z.enum(['free','pro','enterprise']).array().optional()
  4. toggleFeatureFlag(flagKey, enabled, rolloutConfig, adminId) 支持任意 flagKey · 即使 seed 时不存在 · upsert 创建 + 走 dual approval
  5. tests/unit/api/admin/feature-flag-routing.test.ts ≥ 8 it · percentage 0% 全 false / 50% ~50% true / 100% 全 true · targeted user_id 匹配 · plan 匹配 · boolean 简单 toggle · 未知 flagKey upsert+dual
  6. 不破坏 US-012 3 关键紧急开关(都是 boolean flagType)
  7. pnpm audit / test ≥ 1846 / typecheck 全 PASS
  8. Typecheck passes

### [SHIELD] 历史反例 (anti_patterns) — 类似 story 的过往教训, 实现时主动避开:
- **教训** (来自 PRD-13 D-090 复用 / US-013): deterministic hash 同 PRD-13 D-090 复用
  - 反例: `Math.random() < percentage/100`
  - 正确: `parseInt(md5(userId:flagKey).slice(0,8), 16) % 100 < percentage`
- **教训** (来自 PRD-14 US-013 / US-013): rolloutConfig zod discriminated union 严格校验
  - 反例: `accept Record<string, unknown> 任意 JSON`
  - 正确: `z.discriminatedUnion('flagType', [...]) 按 flagType 严格分支`
