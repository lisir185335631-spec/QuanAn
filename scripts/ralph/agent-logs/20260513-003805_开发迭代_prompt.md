...
- **Description**: 作为运营/财务 · 打开 admin.quanqn.com/cost 看月度总成本(同比/环比)+ Top 10 用户横向 bar + 多线折线(模型/Specialist 切换)+ 饼图分项 + 异常告警栏 + CSV 导出按钮。
- **Priority**: 13
- **Retry Count**: 0
- **Acceptance Criteria**:
...
- **教训** (来自 PRD-6 / 数字格式化): 成本数字格式 $ {cost.toFixed(2)} 美元 2 位 · 不 raw float
  - 反例: `<span>{r.cost}</span>  // 显示 0.12345`
  - 正确: `<span>${'$'+r.cost.toFixed(2)}</span>  // $ 0.12`
