...
- **Retry Count**: 1
- **Acceptance Criteria**:
  1. apps/api/src/trpc/routers/privateDomain.ts · 1 procedure(generate)· mock
  2. apps/api/src/trpc/routers/diagnosis.ts · 3 procedure(generate/history/latest)· mock
  3. apps/api/src/trpc/routers/evolution.ts · 7 procedure(evolve/getConfig/updateConfig/history/recentFeedback/feedbackTrend/moduleRanking)· mock
  4. apps/api/src/trpc/routers/deepLearning.ts · 5 procedure(list/create/createFromFile/learn/delete)· mock · 不实涉文件解析(留 PRD-7)
  5. 4 router 全过 zod 校验 · schemas 在 packages/schemas/src/specialist-io/
  6. evolution 7 procedure 全 mock · 实际 EvolutionAgent 留 PRD-8(P7)
  7. Typecheck passes
  8. Tests pass · ≥ 16 unit(总 procedure 数)

### [WARN] 上次验证失败的反馈（优先阅读，针对性修复）：
medium · 流程类 mock · evolution 7 procedure 最多
