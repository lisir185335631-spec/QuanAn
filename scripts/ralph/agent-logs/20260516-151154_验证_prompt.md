...
- **Story ID**: US-005
- **Title**: 首页 WORKFLOW 7 步系统化流程图(横排 + 圆形数字 01-07 + 紫色连接线)
- **当前 retryCount**: 0
- **Acceptance Criteria**:
  1. AC-1 · 新建 apps/web/src/lib/constants/workflow.ts · export WORKFLOW_STEPS · 7 步严格 · [{ num: '01', title: '选择行业', desc: '56 行业精准匹配' }, { num: '02', title: '账号包装', desc: 'AI 生成完整方案' }, { num: '03', title: '变现设计', desc: '三阶梯路径规划' }, { num: '04', title: '学爆款', desc: '全网爆款实时追踪' }, { num: '05', title: '生成文案', desc: '22 元素一键创作' }, { num: '06', title: '制作视频', desc: '分镜表自动生成' }, { num: '07', title: '私域转化', desc: '六大阶段话术覆盖' }]
  2. AC-2 · apps/web/src/pages/Home.tsx 加 WorkflowSection 函数 · 渲染 H2 className="font-display text-4xl font-black text-center text-primary tracking-widest mb-4" 文字 WORKFLOW + p className="font-cn text-center text-sm text-muted-foreground mb-12" 文字 规范流程加上一站式短视频创作系统
  3. AC-3 · WorkflowSection 含 div className="flex flex-col md:flex-row items-stretch justify-between gap-4 md:gap-2" 容器 · WORKFLOW_STEPS.map((step, i) => ...) 渲染 7 步 · 每步含圆形紫色 outline div className="w-12 h-12 rounded-full bg-primary/10 border-2 border-primary/40 flex items-center justify-center mb-3" 内含数字 step.num 紫色 Orbitron font-display + step.title font-cn font-bold + step.desc font-cn text-xs text-muted-foreground
  4. AC-4 · 7 步之间含紫色连接线 div className="hidden md:flex items-center self-center w-8" 内含 div className="w-full h-px bg-primary/20"(md+ 显示 · mobile 隐藏)· i < 6 时渲染连接线
  5. AC-5 · agent-browser 访问 / · 滚到 WORKFLOW · 验证 · H2 紫色 Orbitron 大标题居中 · 7 步 desktop 横排(md+ flex-row · mobile flex-col)· 每步圆形紫色 outline + 数字 01-07 · 中间紫色细连接线(md+ 仅显示)
  6. AC-6 · 截图 screenshots/prd16-us005-home-workflow.png
