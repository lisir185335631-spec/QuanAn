...
- **Description**: 作为用户,如果 56 个行业里没有我的行业,我想点 '自定义输入行业' 弹出 modal 输入自定义行业名 · 保存为 customIndustry · 同样能进入下一步 · 用 shadcn Dialog 严格 click 触发(不允许 hover)
- **Priority**: 7
- **Retry Count**: 0
- **Acceptance Criteria**:
...
[TD-HINT] 执行前 grep .agents/tech-debt.json status=open · 关注 shadcn Dialog 默认行为

### [SHIELD] 历史反例 (anti_patterns) — 类似 story 的过往教训, 实现时主动避开:
- **教训** (来自 prd-16-aiipznt-alignment / US-007): ★ PRD-16 US-007 reject 教训 · shadcn DropdownMenu/Dialog 默认 hover 触发 · 用户期望 click 触发 · 必须 <DropdownMenuTrigger asChild> 或 <DialogTrigger asChild> 包裹 button 才能 click 触发
  - 反例: `<DialogTrigger>{children}</DialogTrigger> (默认 hover) · 或 onMouseEnter={() => setOpen(true)}`
  - 正确: `<DialogTrigger asChild><Button>{label}</Button></DialogTrigger> (asChild + click 触发)`
...
- **US-002**: Step 1 page · Tab 切换 + 搜索 + 56 行业 emoji 双行卡 grid — [OK] 已完成