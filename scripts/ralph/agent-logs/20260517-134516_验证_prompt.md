...
- **Story ID**: US-003
- **Title**: Step 1 已选状态卡 + 主 CTA 跳转
- **当前 retryCount**: 0
- **Acceptance Criteria**:
  1. Step1.tsx 顶部加'已选状态卡'(selectedIndustry 非空时显示)· glass-card border-primary/40 bg-primary/5 + emoji text-3xl + label '已选择:{industry.label}' + 关键词列表 '关键词:{keywords.join('、')}'
  2. 底部主 CTA 用 STEP1_CTA_LABEL 字面 + bg-gradient-to-r from-primary to-primary/80 hover:from-primary/90 hover:to-primary/70 · 严禁 from-violet/from-amber/from-gold(D4=B 红线)· 跟 '→' arrow
  3. disabled 状态 · !selectedIndustry && !customIndustry 时 button disabled + 下方显示 STEP1_CTA_DISABLED_HINT 字面
  4. handleSubmit 逻辑 · localStorage.setItem('acc_step1', JSON.stringify({ industry: selectedIndustry?.id ?? 'other', industryLabel: selectedIndustry?.label ?? customIndustry, customIndustry: customIndustry ?? undefined }))· 然后 navigate('/step/3')
  5. 使用 agent-browser · 打开 /step/1 · 点 '💅 美业' 卡片 · 顶部出现 '已选择:美业' 状态卡 · 关键词显示 '美容院、美发、美甲、美睫、纹绣'
  6. 使用 agent-browser · 底部 CTA '确认并进入下一步 →' 变可点击 · 点 CTA · URL 跳到 /step/3
  7. 使用 agent-browser · 验证 localStorage 存在 'acc_step1' key · 值含 industry: 'beauty'
  8. Typecheck 通过
