...
- **Description**: 作为运维 · 需要 BullMQ cron 每 1h 跑 computeSnapshot · 写入 kpi_snapshots 表 · 前端读 snapshot 不实时跑跨表 join。
- **Priority**: 2
- **Retry Count**: 0
- **Acceptance Criteria**:
...
- **教训** (来自 PRD-9 + D-070 / PRD-9 US-018): BullMQ cron pattern 必加 tz: Asia/Shanghai · 默认 UTC 跟中国时间混乱
  - 反例: `repeat: { pattern: '0 * * * *' }  // 默认 UTC`
  - 正确: `repeat: { pattern: '0 * * * *', tz: 'Asia/Shanghai' }`
- **教训** (来自 PRD-9 + D-070 / BullMQ 复发约束): BullMQ repeat 注册必加 jobId · server 重启每次重注册 → 1h 跑 2 次 SQL · 浪费
  - 反例: `kpiSnapshotQueue.add(name, data, {repeat: {pattern}})  // 漏 jobId`
  - 正确: `kpiSnapshotQueue.add(name, data, {repeat: {pattern, tz}, jobId: "daily-snapshot-recurring"})`
- **教训** (来自 PRD-10 / cron 错峰): 多个 cron 错峰触发 · 防 thundering herd 同 SQL 挤兑
  - 反例: `4 个 cron 全 0 * * * * · 整点全跑`
  - 正确: `kpi 整点 + cost 15 分 + anomaly 30 分 + cleanup 45 分 错峰`
