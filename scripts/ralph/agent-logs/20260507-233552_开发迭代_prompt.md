...
- **Retry Count**: 0
- **Acceptance Criteria**:
  1. shadcn 12 基础组件 copy-paste 到 packages/ui/src/base/*(button / input / dropdown-menu / avatar / dialog / sheet / separator / scroll-area / toast / tooltip / tabs / select)
  2. packages/ui/src/base/index.ts 聚合 export 12 组件
  3. apps/web/src/components/Header.tsx 单文件 · 含 3 dropdown(用户头像 / IP 账号 / 工具入口)· 内联 mock 数据 · 不拆 4 子组件(简化交付)
  4. Header 用户头像 dropdown 显示 mock(name='Mock User' / email='mock@quanqn.dev' / 占位 avatar) + logout 入口
  5. Header IP 账号 dropdown 显示 mock 列表 2-3 个示例账号 + '+ 新建账号' 入口
  6. Header 工具入口 dropdown 显示 14 工具 + 6 新模块文字占位(链接占位 # · 不导航)
  7. apps/web/src/App.tsx 引 Header 替换占位 h1
  8. 0 React warning · 0 hydration mismatch · pnpm typecheck 全 workspace 退出码 0
  9. Typecheck passes
  10. Tests pass(占位 · 无新 unit 必需 · E2E 留 PRD-2)

### [WARN] 上次验证失败的反馈（优先阅读，针对性修复）：
[RCA-002 v0.2 · 断点续跑] 2026-05-07 23:30 拆细 US-005 + 让 ralph 从断点续跑

★ ralph round 6/7/8 (3 次 30min Developer 超时) 实际产出 (在 working tree 未 commit) ·
  · apps/web/package.json · 加 10 @radix-ui deps (avatar/dialog/dropdown-menu/scroll-area/select/separator/slot/tabs/toast/tooltip)
  · pnpm-lock.yaml · 部分更新 (pnpm install 可能没跑完)
  · apps/web/src/components/ui/{12 shadcn 组件}.tsx · 已写
    (button/input/dropdown-menu/avatar/dialog/sheet/separator/scroll-area/toast/tooltip/tabs/select)
  · apps/web/src/components/Header.tsx · 已写
  · apps/web/src/lib/utils.ts · 已写 (cn 函数)
  · apps/web/src/App.tsx · 已改
  · apps/web/src/styles/globals.css · 已改

★ 你的任务 (round 9 fresh start · 不要重写) ·
  1. 跑 pnpm install (装 @radix-ui 10 deps · 可能上次没跑完)
  2. 跑 pnpm --filter @quanqn/web typecheck (修任何 typecheck 错)
  3. 跑 pnpm typecheck (全 workspace 验证零回归)
  4. git add + git commit -m "feat: [US-005] - shadcn 12 基础组件 + Header 简化版 (从 round 6-8 断点续跑)"
  5. 退出 (退出后 ralph.py 会激活 audit-gate 等 Opus 审)

★ 不要 ·
  · 不要重写 12 shadcn 组件 (已存在 · 用 git status 看)
  · 不要重写 Header.tsx
  · 不要在 packages/ui/src/base/ 创建文件 (现实是 shadcn 默认路径 apps/web/src/components/ui/ · 接受这个偏差 · 写到 audit-exemption)

注 · 范围已简化 (size large → medium · 17 文件 → 13 · 移除 4 子组件 + e2e · 留 PRD-2)
...
- **US-004**: Tailwind 引 Aurelian Dark token(从 ui/aurelian_dark/DESIGN.md YAML) — [OK] 已完成