...
- **Description**: part 2 · 批量批准/驳回(max 100)+ auto_review_rules 配置面板(仅 super_admin)+ Tab 切换 [待审核/已批准/已驳回/规则配置]。
- **Priority**: 6
- **Retry Count**: 0
- **Acceptance Criteria**:
...
- **教训** (来自 PRD-12 / AutoRuleConfigPanel 鉴权): AutoRuleConfigPanel 仅 super_admin 可见 · 严格 role 判断
  - 反例: `<AutoRuleConfigPanel /> render 给所有 admin 看`
  - 正确: `{role === 'super_admin' && <AutoRuleConfigPanel />}`
- **教训** (来自 PRD-12 / JSON parse 前端): ruleValue Json editor 前端 parse · 不等后端 400
  - 反例: `submit 后端校验返 400 · 用户体验差`
  - 正确: `try { JSON.parse(value) } catch (e) { setError('JSON 格式错: ' + e.message) }`
...
- **US-005**: reviewTrending UI part 1 · 列表 + 抽屉 framework · P-8 拆 1/2 — [OK] 已完成