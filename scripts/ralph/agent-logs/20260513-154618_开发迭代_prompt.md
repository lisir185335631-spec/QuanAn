...
- **Story ID**: US-013
- **Title**: ★ 收官 · adminRouter index 挂载确认 + admin-routes prd: 12 + 删 2 placeholder + verify-prd-12.sh 9 段
- **Description**: 收官 · adminRouter index 确认挂载 reviewTrending + reviewDeepLearn · admin-routes 2 路由 prd: 12 · 删 2 placeholder · 写 verify-prd-12.sh 9 段含 LD-A-5 grep 防线。
- **Priority**: 13
- **Retry Count**: 0
- **Acceptance Criteria**:
  1. apps/api/src/trpc/routers/admin/index.ts 确认挂载 'reviewTrending: reviewTrendingRouter, reviewDeepLearn: reviewDeepLearnRouter'(US-004+US-009 已加 · 此处 verify)
  2. apps/admin/src/lib/admin-routes.ts 2 路由 metadata.prd 改 12(/admin/reviewTrending + /admin/reviewDeepLearn)
  3. git rm 2 placeholder.tsx · apps/admin/src/pages/admin/placeholder/{reviewTrending,reviewDeepLearn}.tsx
  4. scripts/verify-prd-12.sh 9 段 self-test · §1 workspace · §2 RLS 双向(18/18 admin RLS=false + 15/15 main RLS=true)· §3 audit-redlines ALL PASS · §4 audit-admin-rls AST ≥ 50 procedure 0 violations · §5 PRD-12 文件存在 check · §6 2 placeholder 已删 + 2 路由 prd: 12 · §7 ★ LD-A-5 grep multi-layer(trending-scraper 0 命中 prisma.trendingItem.create + file-parser 0 命中 prisma.deepLearningArchive.create + approve procedure 各 1 命中)· §8 admin-integration 2 URL 200 · §9 typecheck + lint
  5. chmod +x scripts/verify-prd-12.sh · 跑 bash scripts/verify-prd-12.sh · ≥ 30 PASS / 0 FAIL
  6. verify-prd-12.sh fast mode 仅 skip e2e + 浏览器烟测(类 verify-prd-11)· keep 静态 audit
  7. agent-browser 打开 /admin/reviewTrending + /admin/reviewDeepLearn 全 200 · DenseTable virtualScroll 流畅
  8. 8 业务页(PRD-11 6 + PRD-12 2)全部 visible 在 sidebar · 切换均无 console error
  9. Typecheck passes

### [SHIELD] 历史反例 (anti_patterns) — 类似 story 的过往教训, 实现时主动避开:
- **教训** (来自 PRD-12+LD-A-5 / verify §7): verify-prd-N.sh 必含 LD-A-5 grep
  - 反例: `verify 不检 worker 没绕 review queue`
  - 正确: `§7 grep prisma.{trendingItem|deepLearningArchive}.create in workers/ · 必 0 命中`
- **教训** (来自 PRD-10 retro / P-2): admin-routes prd: 12 + placeholder 删
  - 反例: `漏改 metadata.prd`
  - 正确: `prd:12 + git rm`
- **教训** (来自 PRD-11 retro / verify-prd-N.sh fast mode): fast mode 仅 skip e2e + 浏览器 · keep 静态 audit · 防假绿灯
  - 反例: `fast mode skip 全部 audit`
  - 正确: `fast 仅 skip §8 admin-integration + §7 grep 中浏览器 part · keep §1-6 + §9`
...
- **US-006**: reviewTrending UI part 2 · 批量按钮 + 自动规则配置 super_admin · P-8 拆 2/2 — [OK] 已完成
- **US-011**: reviewDeepLearn UI part 2 · 文件预览 + 用户违规累计 Tab · P-8 拆 2/2 — [OK] 已完成
- **US-012**: violation-detection cron + 自动暂停上传 + index.ts wire · ★ Diff-3 验证 — [OK] 已完成