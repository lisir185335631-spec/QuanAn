...
- **Description**: 作为开发者 · invite_campaigns 表 + service · 关联 invite_codes.campaign tag · 转化漏斗计算(注册→激活→9 步完成→D30 留存)。
- **Priority**: 19
- **Retry Count**: 0
- **Acceptance Criteria**:
...
- **教训** (来自 PRD-1+PRD-10 / PRD-1 US-006): adminRLS bypass 用 SET LOCAL
  - 反例: `SET app.role='admin'`
  - 正确: `SET LOCAL app.role='admin'`
- **教训** (来自 PRD-10 retro M-3 / TD-039/042): 新加 admin 表必同步加到 audit-admin-rls-tables.sh
  - 反例: `只改 schema 漏改 audit list`
  - 正确: `同时 ALTER TABLE + 列表 + audit-self-test 跑过`
