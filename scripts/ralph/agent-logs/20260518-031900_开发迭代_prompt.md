...
- **Story ID**: US-001
- **Title**: stepKey 命名规范化 + 老 LS 数据迁移 helper(migrateLegacyLs)
- **Description**: 作为 QuanQn frontend 开发者 · 在 11 step page 接 useStepData 之前 · 需要统一 stepKey 命名(step1/step3/step3b/step4/step4b/step5/step6/step7/step8)+ 一次性迁移 PRD-15~18 老 `acc_step{N}` LS 数据到 LD-009 规范命名(`aiip_memory_acc_{accountId}_step{N}`)· 防数据丢失 + 跨账号隔离立即生效 · App.tsx mount 时 1 次 call · 用 MIGRATION_FLAG_KEY 防重复跑
- **Priority**: 1
- **Retry Count**: 0
- **Acceptance Criteria**:
  1. 新建 apps/web/src/lib/migration/legacy-ls.ts · export migrateLegacyLs(store, accountId): { migrated, skipped } · 内部遍历 LEGACY_KEYS = ['acc_step1', 'acc_step3', 'acc_step3b', 'acc_step4', 'acc_step4b', 'acc_step5', 'acc_step5_selected_topic', 'acc_step6', 'acc_step7', 'acc_step8'] · 读老 key 数据 → 写新 stepLsKey(accountId, stepKey) 或 aiip_memory_acc_{accountId}_selected_topic(跨 step 桥)· 删老 key · 用 MIGRATION_FLAG_KEY = 'aiip_legacy_migration_v1_done' 防重跑(再次 call 返回 { migrated: 0, skipped: 10 })
  2. 新增 apps/web/src/lib/migration/__tests__/legacy-ls.test.ts · 单测覆盖 4 类 edge · (a) 0 legacy keys 存在 → migrated=0, skipped=10, FLAG 写 '1'; (b) 全 10 keys 都有数据 → migrated=10; (c) 部分迁 + 部分新 key 已存在 → 不覆盖新 + 删老; (d) QuotaExceededError 时 console.warn + continue 不中断; (e) accountId === null → return { migrated: 0, skipped: 0 } 不跑
  3. 修改 apps/web/src/App.tsx(或 main.tsx)· mount 时 useEffect 1 次 call migrateLegacyLs(localStorage, activeAccountId) · accountId 通过 useActiveAccount hook 拿(已实施)· 不在 SSR 跑 · 不在 activeAccount 未 ready 跑
  4. performance · 10 keys 迁移 < 50ms(LS 同步 read/write)· 启动时 1 次 call 不影响 FCP > 100ms
  5. test_command · cd apps/web && pnpm vitest run src/lib/migration/legacy-ls.test.ts
  6. Typecheck passes(cd apps/web && pnpm typecheck → 0 error)

### [WARN] 上次验证失败的反馈（优先阅读，针对性修复）：
[TD-HINT] 执行前 grep .agents/tech-debt.json status=open · 列本 story scope (apps/web/src/lib/migration/, apps/web/src/App.tsx) 涉及的 TD · 主动 fix · 不等专项 story · commit message 加 [TD-XXX resolved] tag

### [SHIELD] 历史反例 (anti_patterns) — 类似 story 的过往教训, 实现时主动避开:
- **教训** (来自 prd-15-frontend-completeness / US-002): LD-009 红线 · localStorage key 必须 'acc_' namespace 前缀 · 但 PRD-19 升级到 'aiip_memory_acc_{accountId}_*' 真规范(stepLsKey 现有 helper)· 严禁旧 acc_step{N} 简化命名残留
  - 反例: `localStorage.setItem('acc_step1', JSON.stringify(data))  // 反例 · 缺 accountId · 跨账号串数据`
  - 正确: `import { stepLsKey } from '@/lib/ls-namespace'; localStorage.setItem(stepLsKey(accountId, 'step1'), JSON.stringify(data))  // 正确 · accountId 隔离 LD-009 严守`
- **教训** (来自 prd-19-frontend-backend-bridge / US-001 (self-injected · LS migration 通用风险)): 一次性 migration 必有防重跑标记 · 否则用户每次刷新都跑 · 性能差 + 老 key 已删后重新读返回 null · 误判数据丢失
  - 反例: `export function migrateLegacyLs(store, accountId) { for (const k of LEGACY_KEYS) { ... } }  // 反例 · 无 FLAG 防重跑`
  - 正确: `if (store.getItem(MIGRATION_FLAG_KEY) === '1') return { migrated: 0, skipped: LEGACY_KEYS.length };  // 正确 · 首次跑后 set FLAG · 再次跑 skip`
