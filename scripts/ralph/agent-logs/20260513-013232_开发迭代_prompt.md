...
- **Description**: 作为运维 · 每小时跑 cron 检测 [单用户日 > $5] 异常 · 钉钉 webhook 推送(stub · 真启留 PRR)。dedupe per user per day。
- **Priority**: 15
- **Retry Count**: 0
- **Acceptance Criteria**:
...
- **教训** (来自 PRD-10+D-077 / PRD-10 OAuth multi-layer): webhook/SMTP 默认 isMock=true · 真启走 .env.production
  - 反例: `fetch(webhookUrl) 直接发`
  - 正确: `isMock = process.env.X_ENABLE !== 'true' · isMock 时 log warning`
- **教训** (来自 PRD-9+D-070 / PRD-9 US-018): cron pattern 必加 tz: Asia/Shanghai
  - 反例: `{pattern:'0 * * * *'}  // 默认 UTC`
  - 正确: `{pattern,tz:'Asia/Shanghai',jobId}`
- **教训** (来自 PRD-10 retro / cron 错峰): 多 cron 错峰触发 · 防 thundering herd
  - 反例: `4 cron 全整点 0 * * * *`
  - 正确: `kpi 00 · cost 15 · anomaly 30 错峰`
