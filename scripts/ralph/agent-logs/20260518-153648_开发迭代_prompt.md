...
- **Story ID**: US-005
- **Title**: MonetizationAgent + VideoAgent 真 LLM(for step4b + step6)
- **Description**: 作为 QuanQn 用户 · 在 Step4b 变现规划 + Step6 拍摄计划 · 真接 2 Agent · MonetizationAgent(3 阶梯 + 收入结构 + 案例 输出 schema)+ VideoAgent shooting mode(8 列分镜表 输出 schema · 复杂嵌套)· 防 hardcode model + Schema drift
- **Priority**: 5
- **Retry Count**: 0
- **Acceptance Criteria**:
  1. 修改 apps/api/src/specialists/MonetizationAgent.ts · invokeLLM real call · 3 阶梯 schema(0→90万/100万→1000万/1000万→1亿)严格验证 + 收入结构 + 案例 array 嵌套 schema
  2. 修改 apps/api/src/specialists/VideoAgent.ts · invokeLLM real call · shooting mode 主(production/acquisition mode 暂不动) · 8 列分镜表 schema(时长/场景/景别/角度/运镜/情绪/台词/动作)严格验证
  3. 新建 2 *.real-llm.test.ts · describe.skipIf · 各 1 单测 验证 schema 严格对齐 fallback mock
  4. Schema drift 防御 · MonetizationAgent.threeStages.length === 3 · VideoAgent storyboard 8 列 column 名严格匹配 · safeParse 严格
  5. Verify cost_log 真接 · 2 Agent agentId 真存 + tokens > 0
  6. test_command · cd apps/api && pnpm vitest run src/specialists/__tests__/MonetizationAgent.real-llm.test.ts src/specialists/__tests__/VideoAgent.real-llm.test.ts(若 key 在 env)
  7. Typecheck passes · zero-regression

### [WARN] 上次验证失败的反馈（优先阅读，针对性修复）：
[TD-HINT] 同 US-003/004 · 防 hardcode model + Schema drift · 注意 VideoAgent shooting mode 8 列分镜表 schema 复杂 · 严格 column 匹配

### [SHIELD] 历史反例 (anti_patterns) — 类似 story 的过往教训, 实现时主动避开:
- **教训** (来自 prd-4-p3-ip-flow-9-steps / US-007): Specialist 严禁 hardcode model · 必从 ctx 读 SHIELD REJ-003
  - 反例: `modelUsed: 'claude-sonnet-4-6'  // 反例`
  - 正确: `modelUsed: ctx.actualModel ?? this.config.execution.model_tier`
- **教训** (来自 prd-6-p5-video-modules / US-002 (Storyboard schema)): VideoAgent storyboard schema 必 array 8 列严格 + 字段名严格 · 真 LLM 漂移可能(中文 vs 英文 column name)
  - 反例: `storyboard: [{ '镜头号': 1, ... }]  // 反例 · 中文 key`
  - 正确: `storyboard: [{ shot_id: 1, scene: '...', shot_type: '...' }] · post-process Chinese label render UI 显示`
- **教训** (来自 prd-20-real-llm-integration / US-003): test skipIf 无 key SKIP · CI 不强跑
