# PRD-18 · QuanAn 主应用前端 Step 4 / Step 4b / Step 5 / Step 6 / Step 7 / Step 8 完整化(aiipznt 1:1 对齐 Phase 3)
...
### D4=B 颜色锁(保留 QuanAn globals.css 金色)

继 PRD-17 D-152 / D-158:
- QuanAn globals.css 主色 `--primary: 43 87% 63%`(HSL · hue=43 金色)
...
PRD-18 完成 QuanAn 主应用 9 步主流程剩余 6 step(Step 4 / 4b / 5 / 6 / 7 / 8)的 page 完整化 · 实现严格 1:1 aiipznt 复刻 layout + 字面文字 + UX 流程 · 同时严守 D4=B 金色 token + D3=A 仅主应用边界。
...
```typescript
/**
 * QuanAn · Step 4 执行计划 page 常量 — 字面锁
...
```typescript
/**
 * QuanAn · Step 4b 变现路径 page 常量 — 字面锁
...
```typescript
/**
 * QuanAn · Step 5 爆款选题库 page 常量 — 字面锁
...
```typescript
/**
 * QuanAn · Step 6 拍摄计划 page 常量 — 字面锁
...
```typescript
/**
 * QuanAn · Step 7 文案生成 page 常量 — 字面锁
...
```typescript
/**
 * QuanAn · Step 8 直播策划 page 常量 — 字面锁
