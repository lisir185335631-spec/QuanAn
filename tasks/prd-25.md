...
| **风险分档** | **high × 3**(US-001 /diagnosis 7 维度 LLM 启用 + DiagnosisAgent invokeLLM 完整实施 [重构删: US-002 /voice-chat streaming subscription] · US-003 /daily-tasks AI 任务 + BullMQ worker chain · US-004 /evolution AI 洞察生成 + evolve mutation) + **medium × 3**(US-005 /video-analysis + /analysis · US-006 /video-production [重构删: /acquisition-video] · US-007 /step/8 + /accounts smartRecommend) + **medium × 2**(US-008 TD-090/091 重审 + TD-095 创建并修复 + AGENTS.md §11.16 沉淀 · US-009 收官) |
...
| **LLM 调用(D1=LLM-A · 本 PRD 新解锁 D-225/D-234)** | ✅ **接真 LLM** · 8 page stub → trpc useMutation 替换 · voice-chat + acquisition-video 已删 · DiagnosisAgent 真启用 · /daily-tasks BullMQ worker 触达 · /evolution AI 洞察 + evolve trigger [重构删: /voice-chat streaming] |
| **streaming(SSE)优先** | [重构删: /voice-chat subscription · delta chunks 实时渲染 · voice-chat 已删] · 其他 page 用 mutation(非 streaming · 1 次性 response) |
...
| D-244 | [重构删: /voice-chat subscription protocol 锁] | *(作废 · 语音对话已删)* |
...
| **D-247** | **3 内容创作工具 LLM 输出渲染** · /video-analysis(AnalysisAgent mode='viral' · 5 H3 结构拆解/节奏/爆款/评分/建议)· /analysis(AnalysisAgent mode='structural' · 5 H3 同名复用)· /video-production(VideoAgent mode='production' · 4 H3 分镜脚本/拍摄方案/口播提词器/剪辑指导) [重构删: /acquisition-video VideoAgent mode='acquisition' 3 方案 × 4 H4]· 全部 trpc.{router}.{action}.useMutation · isFallback=true 时显示灰色 fallback marker · history 写入 with agentMode | router 全部已接 specialist · 前端 setSubmitted(true) → useMutation wire up |
...
- ✅ 3 内容创作工具(/video-analysis · /analysis · /video-production)全部 useMutation 替换 setSubmitted(true) · isFallback 灰色 marker · history 写入 *(acquisition-video 已删)*
...
> ⚠️ US-002 作废(2026-06-18 重构)：**语音对话**(`/voice-chat` + `VoiceChatAgent` + STT/TTS worker + streaming subscription)已删除。原内容见 git 历史。
...
### US-006 medium · /video-production 接 VideoAgent 真 LLM(production mode · D-247 部分锁)

> ⚠️ 本 US 部分作废(2026-06-18 重构)：**/acquisition-video**(`获客视频` + `VideoAgent` acquisition mode)已删除。US-006 收缩为仅 /video-production。原 acquisition-video 内容见 git 历史。

**风险分档** · 🟡 **medium**(VideoAgent production mode · 4 H3 输出 · isFallback 处理)

**Story 大小** · small(单次 ralph 迭代可完成 · 2-3 文件修改 · ~100 LOC 新增 + ~15 LOC 删除 stub)

**downstream count** · 0 · risk_level=medium

**前置依赖** · videoProduction router 已接 videoAgent(mode='production') · VideoProduction.tsx stub local state

**用户故事** ·
作为 QuanAn 用户 · 当我在 /video-production 粘贴文案 + 点击 "生成制作方案" button · 看到 loading + 8s 内 4 H3 区块真 LLM 生成(分镜脚本 + 拍摄方案 + 口播提词器 + 剪辑指导)。
...
- **AC-7** · TypeScript typecheck · 0 errors
- **AC-8** · unit test 同步 · `apps/web/src/pages/tools/__tests__/VideoProduction.test.tsx` 更新 ≥ 5 · mock trpc · 验证渲染真数据 *(AcquisitionVideo.test.tsx 已删)*
- **AC-9** · e2e 集成 · `tests/e2e/prd25-content-tools-flow.spec.ts` 追加 · /video-production 1 次 · ≥ 3 assertions *(acquisition-video 已删)*
- **AC-10** · visual diff baseline · `tests/e2e/prd25-visual-baseline.spec.ts` 追加 `prd25-video-production-with-result.png` baseline *(prd25-acquisition-video-with-plans.png 已删)*
- **AC-11** · cost_log 写入 · 验证 db.costLog ≥ 1 条新记录 · agentId='VideoAgent' · agentMode='production' *(acquisition mode 已删)*
- **AC-12** · agent-browser 验证 · 跑 dev server + 浏览器跑 /video-production · 截图 1 张
...
- modify · `tests/e2e/prd25-content-tools-flow.spec.ts`(追加 ≥ 3 assertions)
- modify · `tests/e2e/prd25-visual-baseline.spec.ts`(1 baseline 新建)
...
| US-006 | medium | small | /video-production *(acquisition-video 已删)* | trpc.videoProduction.generate | production | ProductionOutput |
...
US-006 (medium) /video-production [重构删: /acquisition-video] ← 独立 · VideoAgent 接入(仅 production mode)
...
| US-003 high | high | 40-60 min(useQuery + 3 mutation + 轮询 + optimistic UI) | 10-12 min | 3h |
| US-004 high | high | 40-55 min(useQuery + evolve + latestInsight) | 10-12 min | 4.25h |
| US-005 medium | medium | 30-45 min(2 page 复用) | 8-10 min | 5.25h |
| US-006 medium | medium | 30-45 min(2 page 复用) | 8-10 min | 6.25h |
| US-007 medium | medium | 50-70 min(/step8 + /accounts smartRecommend 多文件) | 10-12 min | 8h |
| US-008 medium | medium | 50-75 min(4 TD 处理 + AGENTS 沉淀) | 10-15 min | 9.75h |
| US-009 medium | medium | 40-55 min(verify + retro + handoff) | 8-10 min | 10.75h |
| **累计** | — | **4.5-7.5h daemon** | **76-96 min Opus audit** | **10.5-12.5h wall time** |

### §8.3 跨 PRD-25 大门禁

1. ✅ US-001 完成 = /diagnosis 真 LLM 7 维度评分 · DiagnosisAgent invokeLLM 完整 + isFallback 路径稳
2. ✅ US-003 完成 = /daily-tasks BullMQ 触达 + 轮询 + markCompleted optimistic UI
4. ✅ US-004 完成 = /evolution profile 真数据 + evolve mutation + latestInsight 显示
5. ✅ US-005 完成 = /video-analysis + /analysis 2 工具 page LLM 接入
6. ✅ US-006 完成 = /video-production LLM 接入 *(acquisition-video 已删)*
...
| 成果 | 数据 |
|---|---|
| LLM 接入 pages | 8 pages(diagnosis/daily-tasks/evolution/video-analysis/analysis/video-production/step8/accounts) *(voice-chat + acquisition-video 已删)* |
...
- 重点: DiagnosisAgent(高 CPU reasoning model)
