...
**描述** · 作为内容审核员 / super_admin · 打开 admin.quanan.com/reviewTrending 看待审核 + 已审核列表 + 详情抽屉(rawContent / 自动扫描结果高亮 / 5 平台 filter)· **P-8 大 UI 拆 part 1**(5 files · 严守 plan-check 2.6.14 安全区)。
...
- [ ] H-3 · `ReviewTrendingTable.tsx` · DenseTable from @quanan/ui/admin · 列(queueId/platform/sourceUrl/autoVerdict/status/fetchedAt/操作)· virtualScroll
...
3. **DenseTable 重新撸 div table** · 反例 1000+ 行 div 假表格 · 正例 @quanan/ui/admin DenseTable virtualScroll · D-075
...
**描述** · admin.quanan.com/reviewDeepLearn · 列表 + 抽屉 framework(P-8 拆 1/2)。
...
注入到 · US-005 / US-010 列表 · 严格 import @quanan/ui/admin DenseTable。
