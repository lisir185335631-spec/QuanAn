...
| 项 | 内容 |
|---|---|
| **决策** | VoiceChatAgent / EvolutionAgent / DailyTaskAgent 必须用 **ADR-018 外部 orchestrator 模式** · 不允许 LLM 自己跑循环。VoiceChat = 用户驱动(每次用户说话触发) · Evolution = Cron / 手动触发 · DailyTask = 0 点 Cron。BaseSpecialist `execute()` 单次跑 · 由外部 trigger 重入。 |
| **派生** | ADR-005(L5 自治范式 · 引用 ADR-018) |
| **违规检测** | `audit-ld.sh` LD-004 检测**文件内**是否含**自循环模式**:对 3 个 L5 文件(`apps/api/src/specialists/{VoiceChatAgent,EvolutionAgent,DailyTaskAgent}.ts`)grep `^\s*(for\|while)\s*\(` 上下文 10 行内出现 `llm\|invoke\|complete\|stream.read` · 命中 = self-loop 违规。**不再**用文件名匹配 / 不再要求 grep enqueue(enqueue 检测由 R-3 + bullmq queue 单元测试 cover)。注:TD-052 修复前用"文件名 + grep evolution.queue.add"模式 · 改后聚焦"代码内自循环"才是 ADR-018 真红线。 |
| **例外** | VoiceChatAgent 是流式 · 由用户每说一句触发(也是外部 orchestrator)· 文件存在合法 |
...
| **派生** | ADR-003(同 LD-002) |
| **违规检测** | `audit-ld.sh` LD-005 遍历 `apps/api/src/specialists/*.ts`(排除 `/base/` + `.test.` + `/__tests__/`)· 每个文件必含 `extends BaseSpecialist` · 单元测试必跑。**re-export stub 豁免**:文件含 `from '@/agents/...'` 整行 import 视为 D-007 canonical 重出口(真实现在 `apps/api/src/agents/{evolution,specialists}/`)· 跳过 extends 检查 · 参 TD-024 解决记录。注:2026-05-14 EvolutionAgent stub 已删 · DailyTaskAgent stub 仍在(TD-024 partial)。 |
| **例外** | re-export stub(`from '@/agents/' import` · D-007 + TD-024)· VoiceChatAgent / EvolutionAgent / DailyTaskAgent 本体在 `agents/` 子目录下仍 extends BaseSpecialist |
...
# LD-004 · 3 L5 Agent 走外部 orchestrator (ADR-018) · 检测文件内自循环
# 已对齐 scripts/audit-ld.sh TD-052 修复版 (2026-05-13) — 文件名匹配 → 内容 self-loop 检测
echo "=== LD-004 · L5 Agent 无自循环 ==="
...
for name in "${L5_NAMES[@]}"; do
  ...
  grep -A 10 -E "^\s*(for|while)\s*\(" "$f" 2>/dev/null \
    ...
    | grep -vE "^\s*(\*|//|/\*)" \
    | head -1 | grep -q . && echo "❌ FAIL · ${name}.ts self-loop"
done

# LD-005 · 全部 Specialist 继承 BaseSpecialist · re-export stub 豁免 (D-007 + TD-024)
echo "=== LD-005 · BaseSpecialist 继承 ==="
for f in apps/api/src/specialists/*.ts; do
  [[ "$f" == *base* ]] && continue
  [[ "$f" == *.test.ts ]] && continue
  [[ "$f" == */__tests__/* ]] && continue
  # 跳过 re-export stub (文件含 `from '@/agents/...'`)
  grep -qE "from\s*['\"]@/agents/" "$f" 2>/dev/null && continue
