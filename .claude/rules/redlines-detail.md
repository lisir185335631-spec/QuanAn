---
# 从 AGENTS.md §5 下沉（R7 2026-06-12 · R8 2026-06-18 · R9 2026-06-19 语音下线删 R-23），按需加载
...
22 条红线（R-1~17+R-21~R-22 有 audit 检测·R-18~20 阶段6实装暂未检测·R-23 语音下线删除 2026-06-19）·任一触犯 = Opus 审计直接 reject (hard fail)。
...
检测：`grep -r "new OpenAI\|new Anthropic\|client\.messages\.create\|openai\.chat\.completions" apps/api/src/ packages/*/src/ --exclude-dir=llm-gateway` → 任一命中 reject
...
检测：`grep -rn "Agent\.run\|Agent\.invoke" apps/api/src/specialists/ | grep -v "this\."` → 命中 reject
...
检测：`grep -A20 "execute(input" apps/api/src/specialists/*.ts | grep -E "for|while" | grep "llm"` → 命中 reject（流式 chunk 处理除外）
...
检测：`grep -rn "prisma\.\(stepData\|history\|topic\|asset\|diagnosisReport\|feedbackLog\|evolutionProfile\|evolutionInsight\|deepLearningArchive\|knowledgeFavorite\|knowledgeNote\)\.\(findMany\|findFirst\|findUnique\|update\|delete\)" apps/api/src/ | grep -v "accountId"` → 命中 reject
...
检测：`grep -rE "localStorage\.(setItem|getItem|removeItem)" apps/web/src/ | grep -v "getLsKey\|getToolLsKey\|ls-namespace"` → 0 命中 = OK
...
检测：`tsc --strict --noUncheckedIndexedAccess` 0 error·`grep -rn ": any" apps/api/src/ apps/web/src/ --include="*.ts" | grep -v ".test.ts"` → 命中 reject
...
检测：`grep -rn 'systemPrompt\s*=\s*[` + "`" + `'"]' apps/api/src/specialists/` → 命中 reject
...
错误：在 `apps/api/src/specialists/` 创建 `Step7Agent.ts` / `GenerateAgent.ts` / `BoomGenerateAgent.ts`（同一 Specialist 职责）
正确：都归到 `CopywritingAgent.ts`·用 `mode: 'step7' | 'free' | 'boom'` 分支
检测：`ls apps/api/src/specialists/*.ts | wc -l` ≤ 15(VoiceChat 删后≤14)
...
检测：`grep -rn "#00e5ff\|Orbitron\|Rajdhani" apps/web/src/ apps/admin/src/ packages/ui/src/ --include="*.ts" --include="*.tsx" --include="*.css"` → reject（注释/文档除外）
...
检测：`grep -rn "puppeteer" apps/api/src/workers/trending-scraper/` → reject·`package.json` diff 检查锁定包版本

---

## §5.7 R-23 · VoiceChatAgent finally 软锁（G6·AC-8）【已删除】

> **2026-06-19 语音下线**: VoiceChatAgent 已删除，R-23 对应规则不再适用。原内容保留供历史参考。
...
```bash
pnpm audit:redlines             # 调 scripts/audit-redlines.sh · 22 条红线检测
bash scripts/audit-ld.sh        # 18 LD 检测 + 5 条复杂红线（R-6/R-7/R-8/R-12/R-13）+ R-14 扩展

# 快速单项检查（monorepo 路径）
grep -r "new OpenAI\|new Anthropic" apps/api/src/ packages/*/src/ --exclude-dir=llm-gateway    # R-1
grep -rn "Agent\.run" apps/api/src/specialists/ | grep -v "this\."                             # R-2
grep -rn "prisma\.\w\+\.findMany" apps/api/src/ | grep -v "accountId"                          # R-4
grep -rn "redis\.set\|localStorage\.setItem" apps/api/src/ apps/web/src/ | grep -v "acc_"      # R-5
grep -rn ": any" apps/api/src/ apps/web/src/ --include="*.ts" | grep -v ".test.ts"             # R-10
grep -rn 'systemPrompt\s*=' apps/api/src/specialists/                                          # R-11
grep -rn "#00e5ff\|Orbitron" apps/web/src/ apps/admin/src/ packages/ui/src/                    # R-16
grep -rn "puppeteer" apps/api/src/workers/trending-scraper/                                    # R-17
bash scripts/audit-redlines.sh 2>&1 | grep "R-21"                                              # R-21 trending_favorites RLS
```

22 条红线（R-1~17+R-21~R-22 有 audit 检测·R-18~20 阶段6实装暂未检测·R-23 语音下线删除）·任一命中 → reject。Ralph 提交 PR 前应先跑一遍。
