...
export const INDUSTRY_BY_KEY: Record<string, Industry> = INDUSTRIES.reduce(
  (acc, i) => ({ ...acc, [i.key]: i }),
  {} as Record<string, Industry>,
);

export const INDUSTRIES_BY_CATEGORY: Record<IndustryCategory, readonly Industry[]> = INDUSTRIES.reduce(
  (acc, i) => {
...
/**
 * 行业合规分类(LD-018 · 自动加免责声明的行业)
 */
export const SENSITIVE_INDUSTRIES = {
  medical:  ['health', 'medical', 'psychology', 'fitness'],
  legal:    ['law'],
  finance:  ['finance', 'real_estate'],
} as const;

export function isSensitiveIndustry(key: string): 'medical' | 'legal' | 'finance' | null {
  if ((SENSITIVE_INDUSTRIES.medical as readonly string[]).includes(key)) return 'medical';
  if ((SENSITIVE_INDUSTRIES.legal as readonly string[]).includes(key))   return 'legal';
  if ((SENSITIVE_INDUSTRIES.finance as readonly string[]).includes(key)) return 'finance';
  return null;
}

// === 自检 ===
if (INDUSTRIES.length !== 56) {
  throw new Error(`Expected 56 industries, got ${INDUSTRIES.length}`);
}
