...
| § | 章节 | 一句话 |
|:-:|---|---|
| §1 | **系统总览** | QuanAn 是 aiipznt 的复刻品,本架构在其 spec 之上叠加知识库的 Agent 范式形成"骨架"。 |
...
> **QuanAn = 借用 aiipznt 的产品形态(WHAT)+ 引入知识库的工程哲学(HOW)+ 用 Aurelian Dark 重塑视觉(LOOK)** — 一个"复刻 + 升级 + 范式落地"三位一体的 AI Agent 平台项目。
...
### §1.2 系统的三种本质(从三个角度理解 QuanAn 是什么)
...
│  浏览器侧(三个独立访问入口)                                          │
│                                                                        │
│   www.quanan.com           admin.quanan.com         (移动端 PWA · 复用 web│
│        ↓                         ↓                                      │
│  ┌──────────────┐          ┌──────────────┐                            │
...
| 部署单元 | 域名 | 部署目标 | 何时发版 |
|---|---|---|---|
| **apps/web**(主应用 SPA) | www.quanan.com | Vercel / Cloudflare Pages(静态 + CDN)| 跟随产品迭代 · 周更 |
| **apps/admin**(管理后台 SPA) | admin.quanan.com | Vercel / Cloudflare Pages(独立 · IP 白名单)| 按需 · 月更甚至季度更 |
| **apps/api**(后端服务)| api.quanan.com | Railway / Fly.io(容器 + autoscale)| 跟随两端任一改动 · 但有 PR review |
...
| **9** | **管理后台**(2026-05-07 v0.3 新增 · 对应 REVIEW P2-4) | **1 个 `/invite-manage`(嵌入主应用)+ "推测后台功能"**(spec.md §16.3 实测) | **独立 first-class 子系统** · 16 业务管理域 · 独立部署(admin.quanan.com)· 独立 OAuth · 独立 IP 白名单 · super_admin 强制 MFA · Approval Gates · 独立 9 周路线(P9.0-P9.4) | 原版 admin 简陋是历史包袱 · 复刻产品不应连同包袱一起复刻 · 用户硬约束#2 #3 要求独立 · 详见 [`ADMIN-ARCHITECTURE.md`](ADMIN-ARCHITECTURE.md) |

### §1.8 一句话总结

> 看完本节,你应该知道:**QuanAn 是什么(§1.1)、产品的 3 种本质(§1.2)、3 份资料怎么协同(§1.3)、9 层架构怎么映射(§1.4)、Agent 拓扑长什么样(§1.5)、谁怎么用(§1.6)、跟原版有哪些刻意偏离(§1.7)**。
>
...
管理后台(独立子系统 · 见 ADMIN-ARCHITECTURE.md)
   │
   │ admin SPA(apps/admin)· 部署在 admin.quanan.com
...
| P2 路由地图 | ❌ | 不包含 /admin/* 占位(admin 在 admin.quanan.com 子域名)|
...
❌ 主应用前端 wrapper 检查 role === 'admin' 跳转 admin 页面 · 用户应被拒绝(403 + 引导去 admin.quanan.com)
❌ admin role 用户在主应用看到"额外的 admin 入口" · 主应用不暴露 admin 入口 · admin 用户必须主动访问 admin.quanan.com
```
...
- 这是国内产品罕见的"双范式"应用 · 是 QuanAn 跟 aiipznt 的最大差异之一(§1.7 第 5 条)
...
admin   admin SPA   adminRouter   adminAuth   approvalGate   adminPrisma   admin_audit_log   钉钉
  ...
  │       │
  │       │  ★ 等待 super_admin 在 admin.quanan.com /approval 决策
...
| 维度 | 11/02 mini-program | 05-vertical/02 爆款 | 05-vertical/05 营销 | 02 多 Agent(理论) | **QuanAn(本架构)** |
|---|---|---|---|---|---|
...
| 维度 | 11/02 八 Agent(细) | QuanAn 14 Specialist(中) |
|---|---|---|
...
> ⭐ 关键 · 11/02 是 **「Pipeline 同步流水线」** · QuanAn 是 **「独立调用 + 异步飞轮」** · 但**配置体系和接口标准可以共享**。

### §7.3 跟 02 多 Agent 协作架构的对照(范式选型)

> 02 是理论文档 · 列了 5 种协作模式。QuanAn 选哪种?为什么?
...
#### B · QuanAn 的选择 · **Centralized + 软 Supervisor**(变种)
...
**借鉴点**:
* 05-vertical/02 的"22 爆款元素 + 20 脚本类型"知识体系 → 直接搬到 QuanAn 当常量(§3.6)
* 05-vertical/02 的"AI Prompt 设计模板"思想 → 用在 CopywritingAgent 的多模式分支
* 但**串行流程不搬** — QuanAn 是用户控制 · 不是 Agent 自动跑

### §7.5 跟 05-vertical/05 智能营销的对照(B 端 vs C 端)

> 05-vertical/05 是企业级营销操作系统 · QuanAn 是个人 IP 工具集。两者形态不同。

| 维度 | 05-vertical/05 智能营销 | QuanAn |
|---|---|---|
...
* 05-vertical/05 的"ICP 自学习"思路 → 反馈飞轮的灵感来源(QuanAn 实现得更轻)
...
4 角色 · CEO / CMO / Engineer / Reviewer
   ...
   不适合本场景 ·
     ...
     - QuanAn 是 IP 孵化工具集 · 没有"角色"语义 · 只有"能力"语义
```
...
| 项 | 11/02 八 Agent | QuanAn 14 Specialist | 23 细 Agent(假设) |
|---|:-:|:-:|:-:|
| Prompt 模板数 | 4 | **14** | 23 |
| 配置数 | 8 | 14 | 23 |
...
> 本节回答 · "QuanAn 长什么样" — 不是 Agent 架构,但是项目的"皮肤"。设计稿已经做了 60+ 张 · 本节给出统一的索引和落地方法。
>
> **来源** · [`QuanAn/ui/aurelian_dark/DESIGN.md`](ui/aurelian_dark/DESIGN.md)(项目唯一的设计系统文件 · 7.4KB · YAML 元数据 + 文字描述)
>
...
| § | 章节 | 核心结论 |
|:-:|---|---|
| §1 | 系统总览 | QuanAn = aiipznt 形态 + 知识库哲学 + Aurelian Dark · 三位一体 |
