 ...
 * 34 路由全部可访问 · URL 直输 + 页面不崩溃(无 React error boundary fallback)
 */
...
// 9 step + 14 tools + 6 modules + 3 aux = 32 directly accessible named routes
// Root / (redirect) + catch-all * = 34 total router children
const ROUTES = [
...
test.describe('34 路由可访问性', () => {
  ...
  test('总路由数覆盖 32 命名路由(34 含 index redirect + catch-all)', () => {
    // 9 step + 14 tools + 6 modules + 3 aux = 32 directly navigable routes
    // Plus 1 root index redirect + 1 catch-all = 34 total router children
