// PRD-16 US-011 · prd16-home-flow.test.ts
...
test.describe('PRD-16 Home page flow', () => {
  ...
  test('7 button click navigations', async ({ page }) => {
    ...
    // Buttons 5-7: Function Matrix cards (爆款选题, 文案生成, 呈现形式)
    await page.goto('/');
    await page.getByText('爆款选题').first().click();
    await expect(page).toHaveURL(/\/step\/5/);

    await page.goto('/');
    await page.getByText('文案生成').first().click();
    await expect(page).toHaveURL(/\/step\/7/);

    await page.goto('/');
    await page.getByText('呈现形式').first().click();
