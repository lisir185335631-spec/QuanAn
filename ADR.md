...
QuanQn 是 AI 驱动的 IP 孵化平台 · 含 9 步主向导 / 14 工具页 / 6 新模块 / 反馈飞轮 共 ~30 个节点。每个节点要决定:
...
QuanQn 不是 Pipeline(每用户操作只调 1 个 Specialist) · 但仍需明确层次。
...
QuanQn 产品定位 · "用得越多越懂你"。这需要某种"学习机制" · 让 Specialist 在用户用了 N 次之后输出更贴合用户偏好。
...
QuanQn 每用户可有多个 IP 账号(实测 sally zhao 多账号场景)· 90% 业务数据按 account_id 隔离。这是产品体验**最关键**的安全边界:
...
QuanQn 14 个 Specialist + 3 L5 Agent + ContextAssembler · 都需要调 LLM。如果每处直接 import OpenAI / Anthropic SDK:
...
> **关联 LD** · LD-015
> **范畴** · 视觉(品牌定位)
> **派生自** · [ARCHITECTURE.md §1.7 + §8](ARCHITECTURE.md) · [QuanQn/ui/aurelian_dark/DESIGN.md](ui/aurelian_dark/DESIGN.md)

### Context

aiipznt 原版用赛博青色(`#00e5ff` + `Orbitron`)· 视觉风格强调"科技感 / 未来感"。

但 QuanQn 的目标用户(详见 ADR-001 + [AGENTS §1.3](AGENTS.md)):
...
QuanQn UI 阶段(60+ 设计稿)已经选定 Aurelian Dark 风格(Minimalist Luxury · 金 + 暗) · 本架构需要把这个决策正式 ADR 化。
...
* ADR-001(目标用户决定视觉风格)
* [QuanQn/ui/aurelian_dark/DESIGN.md](ui/aurelian_dark/DESIGN.md)
...
QuanQn 项目从 v0.1 起隐含了前后端分离意图(Vite + Hono + tRPC + 独立的 `pnpm dev` / `pnpm server` 命令)· 但在 v0.2 之前**没有显式架构边界声明**:
...
| D · 多 repo polyrepo | quanqn-web · quanqn-admin · quanqn-api 三个独立仓库 | 大团队 · 跨团队隔离 |

### Decision

**选 C · 多包 monorepo**:
- 三方独立部署单元:`apps/web`(www.quanqn.com)+ `apps/admin`(admin.quanqn.com)+ `apps/api`(api.quanqn.com)
...
- 跨包依赖通过 `workspace:*` 协议(`"@quanqn/schemas": "workspace:*"`)
- 根 `tsconfig.json` 用 path mapping(`"@quanqn/schemas/*": ["packages/schemas/src/*"]`)
...
知识库 [ADR-016 Approval Gates](../Ai_Agent/knowledge-base/08-tech-decisions/) 给出标准范式 · 但 v0.1 版本 QuanQn ADR 18 条**没采纳这条**。
...
- **部署独立** · admin.quanqn.com / 独立 SPA(apps/admin)/ 独立 build / 独立 CI
...
| ⚠️ 团队认知 | 内部团队需理解"为什么访问 admin.quanqn.com 而不是 www.quanqn.com/admin"|
