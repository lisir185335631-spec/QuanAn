...
export interface ScriptType {
  ...
  label: string;
  desc: string;
  /** 给 CopywritingAgent 的 prompt 注入内容 */
  methodology: string;
}
...
export const SCRIPT_TYPES: readonly ScriptType[] = [
  ...
  },
  {
    ...
    emoji: '打',
    label: '打鸡血',
    desc: '励志/激励型内容 · 传递正能量 · 引发共鸣',
    methodology: '打鸡血脚本传递正能量。核心要素 · 1)真实的奋斗故事 · 2)具体的方法论 · 3)激励性的金句 · 4)号召行动。',
  },
] as const;

export const SCRIPT_TYPE_KEYS = SCRIPT_TYPES.map((s) => s.key) as readonly string[];

export const SCRIPT_TYPE_BY_KEY: Record<string, ScriptType> = SCRIPT_TYPES.reduce(
  (acc, s) => ({ ...acc, [s.key]: s }),
  {} as Record<string, ScriptType>,
);

// === 自检 ===
if (SCRIPT_TYPES.length !== 20) {
  throw new Error(`Expected 20 script types, got ${SCRIPT_TYPES.length}`);
}
