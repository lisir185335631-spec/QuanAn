// PRD-29 · DailyTasksPage · /admin/daily-tasks
// KPI 卡(总数/7天新增/fallback占比/平均完成率) + 列表表格(账号/taskDate/完成度/agentId/fallback) + 过滤区
// 过滤采用 committed 模式：所有项存"草稿 state"，点「筛选」后才同步查询 state + reset page=1
// 点开 detail 抽屉(显示 tasks JSON，<pre> maxHeight+overflow 防大内容卡死)
// 暗金视觉风格 (admin 标准 var(--gold) / var(--bg-panel) / var(--border))
// SHIELD: adminTrpc.dailyTasks.list/detail/kpiStats — no mock data
...
function fmtDateOnly(d: Date | string): string {
  // taskDate 是 @db.Date 列，取 ISO 前 10 字符避免 toLocaleDateString 时区偏移
  const iso = new Date(String(d)).toISOString(); // always UTC
...
function DetailDrawer({ recordId, onClose }: { recordId: number; onClose: () => void }) {
  const { data, isLoading, isError } = adminTrpc.dailyTasks.detail.useQuery(
    { id: recordId },
    { staleTime: 60_000 },
  );
      ...
      {data && (
        <>
          ...
          <div style={{ marginBottom: 20 }}>
            ...
            <MetaRow label="任务日期 (taskDate)" value={fmtDateOnly(data.taskDate)} highlight />
            <MetaRow label="完成进度" value={`${data.completedCount} / ${data.totalCount}`} />
            <MetaRow label="Agent ID" value={data.agentId} mono />
            <MetaRow label="模型 (modelUsed)" value={data.modelUsed ?? '—'} mono />
          ...
          {/* Tasks JSON — <pre> with maxHeight+overflow to prevent large content freeze */}
          <div style={{ marginBottom: 20 }}>
            <div
              style={{
                fontSize: 11,
...
function draftToCommitted(d: FilterDraft): FilterCommitted {
  ...
  if (d.isFallback === 'true') isFallback = true;
  else if (d.isFallback === 'false') isFallback = false;
  return {
    accountId,
...
export default function DailyTasksPage() {
  ...
  const { data: kpi, isLoading: kpiLoading } = adminTrpc.dailyTasks.kpiStats.useQuery(undefined, {
    staleTime: 30_000,
  });
  ...
  const {
    ...
    isError,
    refetch,
  } = adminTrpc.dailyTasks.list.useQuery(
    {
      page,
      pageSize: PAGE_SIZE,
      accountId: committed.accountId,
      agentId: committed.agentId,
      isFallback: committed.isFallback,
    ...
    <div style={{ position: 'relative' }}>
      ...
      >
        ...
        <div style={{ fontSize: 12, color: 'var(--text-muted)', marginTop: 4 }}>
          全账号 DailyTask 表 · AI 每日任务生成记录(agentId/modelUsed/isFallback) · 共 {total.toLocaleString()} 条记录
          <button
            type="button"
      ...
      <div style={{ display: 'flex', gap: 12, marginBottom: 20, flexWrap: 'wrap' }}>
        {kpiLoading ? (
          ...
          <>
            ...
            <KpiCard
              label="Fallback 占比"
              value={fmtPct(kpi?.fallbackRate ?? 0)}
            />
            <KpiCard
              label="平均完成率"
              value={fmtPct(kpi?.avgCompletionRate ?? 0)}
            />
      ...
      >
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
          <label style={{ fontSize: 12, color: 'var(--text-dim)' }}>Agent ID</label>
          <input
            type="text"
            value={draft.agentId}
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
          <label style={{ fontSize: 12, color: 'var(--text-dim)' }}>Fallback</label>
          <select
            value={draft.isFallback}
            onChange={(e) => setDraft((d) => ({ ...d, isFallback: e.target.value }))}
            style={{
              background: 'var(--bg)',
              border: '1px solid var(--border)',
              color: 'var(--text-muted)',
      ...
      {!isLoading && !isError && (
        <div style={{ overflowX: 'auto' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12 }}>
            <thead>
              <tr>
                {['ID', '账号 ID', '任务日期', '完成度', 'Agent ID', '模型', 'Fallback', '创建时间'].map(
                  (col) => (
                    <th
                      key={col}
                      style={{
                  ...
                  <td style={{ padding: '8px 10px' }}>
                    <span
                      style={{
                        ...
                        borderRadius: 3,
                        padding: '1px 6px',
                        color: row.completedCount >= row.totalCount && row.totalCount > 0
                          ? 'var(--gold)'
                          : 'var(--text-muted)',
                        fontFamily: 'monospace',
                        fontSize: 11,
